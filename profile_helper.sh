...
if [ -f $XDG_CONFIG_HOME/base16_theme ]; then
  script_name=$(basename "$(realpath $XDG_CONFIG_HOME/base16_theme)" .sh)
  echo "export BASE16_THEME=${script_name#*-}"
  echo ". $XDG_CONFIG_HOME/base16_theme"
fi
cat <<'FUNC'
_base16()
{
  ...
  local theme=$2
  [ -f $script ] && . $script
  ln -fs $script $XDG_CONFIG_HOME/base16_theme
  export BASE16_THEME=${theme}
  echo -e "if \0041exists('g:colors_name') || g:colors_name != 'base16-$theme'\n  colorscheme base16-$theme\nendif" >| $XDG_CONFIG_HOME/vim/vimrc_background
